...
:   QGeoMappingManagerEngine()
{
    *error = QGeoServiceProvider::NoError;
    ...
    if (parameters.contains(QStringLiteral("maplibre.china"))) {
        m_useChinaEndpoint = parameters.value(QStringLiteral("maplibre.china")).toBool();
    }

    if (parameters.contains(QStringLiteral("maplibre.api_base_url"))) {
        const QString apiBaseUrl = parameters.value(QStringLiteral("maplibre.api_base_url")).toString();
    ...
    if (parameters.contains(QStringLiteral("maplibre.mapping.additional_style_urls"))) {
        const QString ids = parameters.value(QStringLiteral("maplibre.mapping.additional_style_urls")).toString();
    ...
    if (parameters.contains(QStringLiteral("maplibre.api_key"))) {
        m_settings.setApiKey(parameters.value(QStringLiteral("maplibre.api_key")).toString());
    }

    bool memoryCache = false;
    if (parameters.contains(QStringLiteral("maplibre.mapping.cache.memory"))) {
        memoryCache = parameters.value(QStringLiteral("maplibre.mapping.cache.memory")).toBool();
        m_settings.setCacheDatabasePath(QStringLiteral(":memory:"));
    }

    QString cacheDirectory;
    if (parameters.contains(QStringLiteral("maplibre.mapping.cache.directory"))) {
        cacheDirectory = parameters.value(QStringLiteral("maplibre.mapping.cache.directory")).toString();
    } else {
    ...
    if (parameters.contains(QStringLiteral("maplibre.mapping.cache.size"))) {
        bool ok = false;
        int cacheSize = parameters.value(QStringLiteral("maplibre.mapping.cache.size")).toString().toInt(&ok);

        if (ok)
            m_settings.setCacheDatabaseMaximumSize(cacheSize);
    }

    if (parameters.contains(QStringLiteral("maplibre.mapping.use_fbo"))) {
        m_useFBO = parameters.value(QStringLiteral("maplibre.mapping.use_fbo")).toBool();
    }

    if (parameters.contains(QStringLiteral("maplibre.mapping.items.insert_before"))) {
        m_mapItemsBefore = parameters.value(QStringLiteral("maplibre.mapping.items.insert_before")).toString();
    }
